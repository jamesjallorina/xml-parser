...
// the text before generation.
{
    ...
    //std::cout << "GGGGGGGGGGGGGGGGGGGGGGGGGGG 1 " << strNormalizedText << endl;
    if(!xml.MustPreserve())
    {
		...
		//////////////////////////////////
		if (strNormalizedText.substr(0, 2) == "(;") // don't truncate but remove # from first char
		{
			strNormalizedText = strNormalizedText.substr(2, strNormalizedText.size());
		}
		else
		{
			Private::NormalizeWhiteSpaces(strNormalizedText);
